...
(display "This file contains sample code from March 23, 2026,\n")
...
(define (multiply k)
  (cond
    ...
    [else         (error "multiply: arg is not a number")]
    )
  )

; Try:
;   (define f (multiply 2))
;   (define g (multiply 10))
;   (f 7)
;   (g 7)
;   ((multiply 5) 6)
...
(define (my-filter p xs)
  (cond
    ...
    [else         (error "my-filter: arg #2 is not a list")]
    )
  )
...
; Try:
;   (my-filter even? '(1 4 8 7 11 100))
;   (my-filter string? '(1 "abc" (1 2 3) "def"))
;   (my-filter string? 3)
    ...
    (let* (
      ...
      (cond
        [num   (begin
                 ...
                 (display (* num num))
                 (newline))]
        [else  (display "You did not type a number, you nincompoop!\n")]
        )
      )
    )
  )
...
; k, k+1, k+2, etc.
(define (count-from k)
  (cons (delay k) (delay (count-from (+ 1 k))))
  )

; posints
; Infinite lazy list (see above) with items 1, 2, 3, etc.
(define posints (count-from 1))
...
; Scheme has "take", which is like Haskell's "take", but the arguments
