...
main = do
    putStrLn ""
    putStrLn "This file contains sample code from February 25, 2026,"
...
-- The idea used above is called corecursion: produce a stream of values
-- recursively, ending the stream when there is no more need for values.
...
-- bigger
-- Return the larger of two values, or the first if they are the same.
...
-- maxVal
-- Return the maximum value in a nonempty list. Uses bigger.
