...
main = do
    putStrLn ""
    putStrLn "This file contains sample code from February 23, 2026,"
...
--   listLength "Hello!"
-- This operation is available as the Prelude function "length".
--   length [1000..10000]
--   length "Hello!"
...
-- Try:
--   myConcat "aard" "vark"
-- This operation is available as the Prelude operator "++".
-- Try:
--   "aard" ++ "vark"
...
-- Try:
--   myMap square [1..10]
-- This operation is available as the Prelude function "map".
-- Try:
--   map square [1..10]

-- Note that map can always be replaced by a list comprehension.
...
-- if-then-else does pretty much what you expect, except that it is an
-- expression. In "if COND then EXPR1 else EXPR2", COND must have type
-- Boolean, and EXPR1 and EXPR2 must have the same type. The value is
-- that of EXPR1 is COND evaluates to True, and the value of EXPR2 if
-- COND evaluates to False.

-- Try:
--   myFilter (\x -> mod x 2 == 0) [1..10]
-- This operation is available as the Prelude function "filter".
-- Try:
--   filter (\x -> mod x 2 == 0) [1..10]

-- Note that filter can always be replaced by a list comprehension.
