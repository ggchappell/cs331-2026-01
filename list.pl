...
% Try:
%   ?- concat([1,2], [3,4,5], X).
%   ?- concat(X, [3,4,5], [1,2,3,4,5]).
%   ?- concat([1,2], Y, [1,2,3,4,5]).
%   ?- concat([A,2], [C,4,5], [1,B,3,4,5]).
%   ?- concat(X, Y, [1,2,3,4,5]).
...
% filter/2
filter(_, [], []).
filter(P, [H|T], [H|L]) :- call(P, H), filter(P, T, L).
...
% Try:
%   ?- map(threeMore, [2,6,10], X).
%   ?- map(threeMore, X, [2,6,10]).
%   ?- map(aBitMore, [10,20], X).
%   ?- map(aBitMore, X, [10,20]).
%   ?- map(aBitMore, [X,20], [10,Y]).
