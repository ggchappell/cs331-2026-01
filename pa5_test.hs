...
-- test_filterOther
-- Test Suite for function filterOther
test_filterOther = do
    testMsg "Test Suite: Function filterOther"
    testEq (PA5.filterOther (>0) [-1,1,-2,2] [1,2,3,4,5,6]) [2,4]
        "filterOther, example #1 from Assn 5 description"
    testEq (PA5.filterOther (==1) [2,2,1,1,1,1,1] "abcde") "cde"
        "filterOther, example #2 from Assn 5 description"
    testEq (PA5.filterOther (=='z') "azazazazazaz" [1..11]) [2,4..10]
        "filterOther, 1st list is string"
    testEq (PA5.filterOther (\n -> mod n 4==2) [1..100] "spaghetti")
        "pe"
        "filterOther, 2nd list is string"
    testEq (PA5.filterOther (\n -> mod n 3==1) [2..100000] [6,8..4000])
        [10,16..4000]
        "filterOther, longer lists #1"
    testEq (PA5.filterOther (\n -> mod n 3==1) [2..4000] [6,8..100000])
        [10,16..8002]
        "filterOther, longer lists #2"
    testEq (PA5.filterOther (\n -> n>1000 && n<2000) [3,7..3000]
            [4,9..5000])
        [1254,1259..2499]
        "filterOther, longer lists #3"
    testEq (PA5.filterOther (>0) [] [1..100000]) []
        "filterOther, first list empty"
    testEq (PA5.filterOther (\n -> mod n 2==0) [1..100000] noints) []
        "filterOther, second list empty"
    testEq (PA5.filterOther (>0) [] noints) []
        "filterOther, both lists empty"
    where
        noints = []::[Integer]
...
-- test_sublist
-- Test Suite for function sublist
test_sublist = do
    testMsg "Test Suite: Function sublist"
    testEq (PA5.sublist "cde" "abcdefg") (Just 2)
        "sublist, example #1 from Assn 5"
    testEq (PA5.sublist "cdX" "abcdefg") Nothing
        "sublist, example #2 from Assn 5"
    testEq (PA5.sublist [1] [2,1,2,1,2]) (Just 1)
        "sublist, example #3 from Assn 5"
    testEq (PA5.sublist [] [1,2,3,4,5]) (Just 0)
        "sublist, example #4 from Assn 5"
    testEq (PA5.sublist [20..25] [3..100]) (Just 17)
        "sublist, longer list #1: found"
    testEq (PA5.sublist ([20..25]++[1]) [3..100]) Nothing
        "sublist, longer list #2: not found"
    testEq (PA5.sublist ([20..25]++[1]) [3..24]) Nothing
        "sublist, longer list #3: not found at end"
    testEq (PA5.sublist ([20..25]++[1]) ([3..30]++[20..25]++[1]))
        (Just 28)
        "sublist, longer list #4: found 2nd time"
    testEq (PA5.sublist ([50..100]++[1..50])
            (h++[1..49]++[49..100]++[10..101]++h++h++h++h))
        (Just 342)
        "sublist, longer list #5"
    where
        h = [1..100]


-- test_concatAlternate
-- Test Suite for function concatAlternate
test_concatAlternate = do
    testMsg "Test Suite: Function concatAlternate"
    testEq (PA5.concatAlternate ["aa","bb","cc","dd","ee"])
        ("aaccee","bbdd")
        "concatAlternate, example #1 from Assn 5"
    testEq (PA5.concatAlternate ["dog","zebra"]) ("dog","zebra")
        "concatAlternate, example #2 from Assn 5"
    testEq (PA5.concatAlternate nostrs) ("","")
        "concatAlternate, example #3 from Assn 5"
    testEq (PA5.concatAlternate ["hello"]) ("hello","")
        "concatAlternate, example #4 from Assn 5"
    testEq (PA5.concatAlternate
              ["z","y","z","y","z","y","z","y","z","y"])
        ("zzzzz","yyyyy")
        "concatAlternate, example #5 from Assn 5"
    testEq (PA5.concatAlternate ["interface","collaboration","ostrich"])
        ("interfaceostrich","collaboration")
        "concatAlternate, some words"
    testEq (PA5.concatAlternate (take 2001 (longlist "abc")))
        (catem "abc" 1001, catem "abc" 1000)
        "concatAlternate, longer list"
    where
        nostrs = []::[String]
            -- Empty list that will not confuse the type inference
...
main = do
    ...
    if allPassed allTests then do
        putStrLn ""
        putStrLn "  :::::::::::::::::::::::::::::::::::::::::::::::::::"
        putStrLn "  : Note. concatAlternate must be done as a fold.   :"
