...
local tamandua_state  -- Tamandua variable values
...
-- Tamandua program.
-- Uses local seeded (set to false in runTamandua), seed.
local seeded = false
local seed = 0
function tamandua_util.random(n)
...
local function printHelp()
    io.write("Type Tamandua code to execute it.\n")
    io.write("Commands (these may be abbreviated;")
    io.write(" for example, \":e\" for \":exit\")\n")
    io.write("  :exit          - Exit.\n")
    io.write("  :run FILENAME  - Execute Tamandua source file.\n")
    io.write("  :clear         - Clear Tamandua state")
...
-- clearState
-- Clear Tamandua state: functions, simple variables, arrays.
local function clearState()
    tamandua_state = { f={}, v={}, a={} }
end


-- runTamandua
-- Given a string, attempt to treat it as source code for a Tamandua
-- program, and execute it. I/O uses standard input & output.
--
-- Parameters:
--   program  - Tamandua source code
--   state    - Values of Tamandua variables as in interpit.interp.
...
local function runTamandua(program, state, exec_msg)
    ...
    if good and done then
        ...
        newstate = interpit.interp(ast, state, tamandua_util)
    else
        newstate = state
    end
...
-- runFile
-- Given filename, attempt to read source for a Tamandua program from
-- it, and execute the program. If prinntmsg is true and the program
-- parses correctly, then print a message before executing the file.
local function runFile(fname, printmsg)
    ...
    if not readable(fname) then
        errMsg("Tamandua source file not readable: '"..fname.."'")
        return
    end
    local source = ""
    ...
    good, done, tamandua_state =
        runTamandua(source, tamandua_state, exec_msg)
    if not (good and done) then
        errMsg("Syntax error in Tamandua source file: '"..fname.."'")
    end
end
...
local function doReplCommand(line)
    local suffix = "tdua"  -- Source filename suffix
    ...
    elseif line:sub(1,2) == ":c" then
        clearState()
        io.write("Tamandua state cleared\n")
        return true
...
-- repl
-- Tamandua REPL. Prompt & get a line. If it begins with a colon (":")
-- then treat it as a REPL command. Otherwise, treat line as Tamandua
-- program, and attempt to execute it. If it looks like an incomplete
-- Tamandua program, then keep inputting, and continue to attempt to
-- execute. REPEAT.
local function repl()
    ...
    while true do
        ...
        if line:sub(1,1) == ":" then    -- Command
            ...
            source = source .. line
            good, done, tamandua_state =
                runTamandua(source, tamandua_state)
            if (not good) and done then
            ...
            if not done then
                errMsg("Syntax error in Tamandua code")
            end
        end
    end
end
...
-- Command-line argument? If so treat as Tamandua source filename, read
-- source, and execute.
if arg[1] ~= nil then
    ...
    io.flush()  -- Ensure previous output is done before input
    io.read("*l")
-- Otherwise, fire up the Tamandua REPL.
else
    repl()
end
